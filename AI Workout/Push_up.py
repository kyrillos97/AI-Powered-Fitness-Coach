...
class Workout:
    ...
    # -------------------- Validation --------------------
    def _validate_push_up(self, angle: float,angle2:float) -> bool:
        if 80 <= angle <= 110 and 60 <= angle2 <= 110:
            return True
        if angle > 60 and angle2 > 60:
            self.feedback_handler.give_feedback("push_up", "Bend your elbows more")
            return False
        if angle < 60 and angle2 < 60:
    ...
    def rest_period(self, duration,numset,tarset, is_exercise_rest=False):
        ...
        while time.time() < end_time:
            ...
            cv2.putText(frame, txt, (50, 200), cv2.FONT_HERSHEY_SIMPLEX,
                        1.0, (0, 0, 255), 3)
            if label !="exercise":
              txt = f"Push Up | Set {numset}/{tarset} and the remaining set : {tarset-numset}"
        ...
        for ex_idx, (exercise_name, rep_groups) in enumerate(exercise_plan.items()):
            ...
            for group_idx, target_reps in enumerate(rep_groups):
                ...
                while rep_count < target_reps:
                    ...
                    if phase == "up":
                        if self._validate_push_up(angle,angle2) and body_straight:
                            phase = "down"
                    elif phase == "down":
                        if angle > 110 and angle2 > 110 and body_straight:
                            rep_count += 1
                    ...
                    cv2.putText(frame, f"Elbow Angle: {avg_angle:.1f}", (10, 90),
