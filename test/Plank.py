...
class Workout:
    ...
    def _get_landmarks(self, results):
        ...
        critical_keypoints = [5,6,7,8,9,10,11,12,15,16]
        if len(finalres) != 0:
    ...
    def _is_body_straight(self, lm, threshold=30):
    ...
    # -------------------- Validation --------------------
    def _validate_plank(self, angle: float,angle2:float) -> bool:
        if 150 <= angle <= 180 and 150 <= angle2 <= 180:
            return True
        self.feedback_handler.give_feedback("plank", "Straighten your arms")
    ...
    def rest_period(self, duration,numset,tarset, is_exercise_rest=False):
        ...
        while time.time() < end_time:
            ...
            cv2.putText(frame, txt, (50, 200), cv2.FONT_HERSHEY_SIMPLEX,
                        1.0, (0, 0, 255), 3)
            if label !="exercise":
              txt = f"Plank | Set {numset}/{tarset} and the remaining set : {tarset-numset}"
        ...
        for ex_idx, (exercise_name, hold_durations) in enumerate(exercise_plan.items()):
            ...
            for group_idx, target_hold in enumerate(hold_durations):
                ...
                while hold_elapsed < target_hold:
                    ...
                    if lm:
                        angle = self._compute_angle_for_triplet(lm, (6, 8, 10))
                        angle2 = self._compute_angle_for_triplet(lm, (5, 7, 9))
                        body_straight = self._is_body_straight(lm)

                    current_time = time.time()
                    is_good_form = self._validate_plank(angle, angle2) and body_straight
                    ...
                    cv2.putText(frame, txt, (10, 60), cv2.FONT_HERSHEY_SIMPLEX,
                                0.8, (0, 255, 0), 2)
                    avg_angle = (angle + angle2) / 2
                    cv2.putText(frame, f"Arm Angle: {avg_angle:.1f}", (10, 90),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                    straight_txt = "STRAIGHT" if body_straight else "ALIGN"
                    cv2.putText(frame, f"Body: {straight_txt}", (10, 110),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0) if body_straight else (0, 0, 255), 1)
                    ready_txt = "CONFIRMED" if self.person_confirmed else "NOT CONFIRMED"
                    cv2.putText(frame, f"Person: {ready_txt}", (10, 120),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
                    form_txt = "GOOD" if is_good_form else "BAD"
                    cv2.putText(frame, f"Form: {form_txt}", (10, 140),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0) if is_good_form else (0, 0, 255), 1)
